...
[TestFixture]
internal sealed class UntilCanceledTests : CodeFixVerifier
{
	[Test]
	...
	public void InvalidUsage()
	{
		...
		var expected = new DiagnosticResult
		{
			...
			Locations = [new DiagnosticResultLocation("Test0.cs", s_preambleLength + 7, 46)],
		};
	...
	public void InvalidMultiLineUsage()
	{
		...
		var expected = new DiagnosticResult
		{
			...
			Message = "UntilCanceled() may only be used in methods that return IEnumerable<AsyncAction>",
			Severity = DiagnosticSeverity.Warning,
			Locations = [new DiagnosticResultLocation("Test0.cs", s_preambleLength + 8, 19)],
		};
